...
(function() {
    ...
    class ApiService {
        ...
        async fetchItems() {
            try {
                const request = new Request(this.url + '/db.json', {
                    method: 'get'
                });
    ...
    function buttonNavClick(evt) {
        ...
        if (this.$el.classList.contains('catalog__button-nav')) {
            catalog.active();
        } else {
            nav.active();
        }
    ...
    class Catalog extends Component {
        ...
        active() {
            this.$el.classList.toggle('catalog_active');
        }
    ...
    class CatalogListComponent extends Component {
        ...
        async init() {
            ...
            const html = data.map(item => fragment.appendChild(renderCatalogItem(item)));
            this.$el.appendChild(fragment);
            this.$el.addEventListener('click', catalogListClick);
            window.addEventListener('mousemove', catalogListMousemove);
        }
    }

    function catalogListClick(evt) {
        evt.preventDefault();
        if (evt.target.classList.contains('catalog-item__link')) {
    ...
    function catalogListMousemove(evt) {
        if (evt.target.classList.contains('catalog-item__link')) {
            let dx = 0;
            let dy = 0;
            let rS = 10;
    ...
    // render-catalog-item.js
    const templateCatalogItem = document.querySelector('#t-catalog-item').content.querySelector('.catalog-item');

    function renderCatalogItem(item) {
        const element = templateCatalogItem.cloneNode(true);

        element.querySelector('.catalog-item__link').textContent = item.title;
        const elementTitle = element.querySelector('.catalog-item__title');
        elementTitle.textContent = item.title;
        element.querySelector('.catalog-item__text').textContent = item.text;
        ...
        elementPicture.classList.add('picture', 'catalog-item__picture');
    ...
    const catalogButtonNav = new ButtonNav('.catalog__button-nav');
    const modalButtonClose = new ButtonClose('.modal__button-close');
    const catalog = new Catalog('.catalog');
    const catalogList = new CatalogListComponent('.catalog-list');
